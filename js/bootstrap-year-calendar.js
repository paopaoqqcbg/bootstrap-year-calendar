...
(function($) {
	...
	Calendar.prototype = {
		...
		_initializeEvents: function(opt) {
			...
			if (opt.currentYearChanged) {
				this.element.bind('currentYearChanged', opt.currentYearChanged);
			}
			if (opt.renderDay) {
		...
		getWeekNumber: function(date) {
			var firstDayOfYear = new Date(this.options.startYear, 0, 1);
			var d = dates[this.options.language].weekStart;
			var daydiff = firstDayOfYear.getDay() - d;
			var firstWeekDayOfYear = new Date(firstDayOfYear);
			firstWeekDayOfYear.setDate(firstWeekDayOfYear.getDate() - daydiff);
			var tempDate = new Date(date.getTime());
			tempDate.setHours(0, 0, 0, 0);
			var getWeekNo = ((tempDate.getTime() - firstWeekDayOfYear.getTime()) / 86400000) / 7;
			// tempDate.setDate(tempDate.getDate() + 3 - (tempDate.getDay() + 6) % 7);
			// var week1 = new Date(tempDate.getFullYear(), 0, 1);
			// var getWeekNo = 1 + Math.round(((tempDate.getTime() - week1.getTime()) / 86400000 - 3 + (week1.getDay() + 6) % 7) / 7);
			// if (tempDate.getFullYear() < this.options.startYear) {
			// 	getWeekNo = 1
			// } else if (tempDate.getFullYear() > this.options.startYear) {
			// 	getWeekNo++;
			// }
			return getWeekNo + 1;
		},
		...
		setYear: function(year) {
			...
			if (!isNaN(parsedYear)) {
				...
				this._render();
				this._triggerEvent('currentYearChanged', {
					currentYear: this.options.startYear
				});
			}
		},
	...
	$.fn.currentYearChanged = function(fct) {
		$(this).bind('currentYearChanged', fct);
	}
